...
export function RatingBar({ editorial, users, external }: RatingBarProps) {
  return (
    <div className="flex items-center gap-2">
      {/* Użytkownicy */}
      {users && users.average != null && (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-1">
                <Star className="h-4 w-4 fill-amber-400 text-amber-400" />
                <span className="font-semibold">{(users.average ?? 0).toFixed(1)}</span>
      ...
      {/* Redakcja */}
      {editorial && editorial.average != null && (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-1">
                <Star className="h-4 w-4 fill-violet-400 text-violet-400" />
                <span className="font-semibold">{(editorial.average ?? 0).toFixed(1)}</span>
      ...
      {/* Zewnętrzne */}
      {external && external.average != null && (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-1">
                <Star className="h-4 w-4 fill-cyan-400 text-cyan-400" />
                <span className="font-semibold">{(external.average ?? 0).toFixed(1)}</span>
